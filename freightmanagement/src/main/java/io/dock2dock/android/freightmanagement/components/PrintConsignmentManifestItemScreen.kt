...
import io.dock2dock.android.freightmanagement.viewModels.PrintConsignmentManifestItemViewModel
...
@Composable
fun PrintConsignmentManifestItemScreen(consignmentManifestId: String, onDismissRequest: () -> Unit)
{
    val viewModel = viewModel {
        PrintConsignmentManifestItemViewModel(consignmentManifestId) {
            onDismissRequest()
        }
    }
        ...
        Column(modifier = Modifier.padding(16.dp)) {
            ...
            Column(
                ...
                FormItem("Consignment Item") {
                    Dock2DockDropdown(
                        options = viewModel.consignmentManifestItems,
                        selectedTextExpression = { it.description },
                        selectedText = viewModel.consignmentManifestItemName,
                        errorMessage = viewModel.consignmentHeaderItemErrorMessage,
                        isError = viewModel.consignmentManifestItemIsError,
                        placeholderText = "--Select--",
                        selectedItemChanged = {
                            viewModel.onConsignmentManifestItemValueChanged(it)
                        }
                    )
                    {
...
@Composable
internal fun PreviewPrintConsignmentManifestScreen() {
    val context = LocalContext.current
    Dock2DockConfiguration.init(context, "")

    PrintConsignmentManifestItemScreen(
        "12345"
    ) {}
}